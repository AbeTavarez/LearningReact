class App extends React.Component {
  ...
  async componentDidMount() {
    const req = await fetch(
      "https://api.giphy.com/v1/gifs/trending?api_key=1Gyc9KesGWa4YGiIjReLXOojU5mdNfBV"
    );
      ...
      >
        ...
        <div>
          {this.state.data.length && (
            <img
              src={this.state.data[this.state.ranNum].images.downsized.url}
              alt="photo"
            />
          )}
        </div>
